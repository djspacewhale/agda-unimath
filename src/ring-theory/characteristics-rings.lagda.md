...
```agda
exists-decidable-characteristic-Ring : {l : Level} (R : Ring l) → is-decidable-subtype (subset-ideal-Ring ℤ-Ring (characteristic-Ring R)) → {!   !}
exists-decidable-characteristic-Ring R R-dec-char = Σ ℕ λ n → characteristic-Ring R ＝ {!   !}
```
